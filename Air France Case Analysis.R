...
#Directory of the Air France Data set
AirFr_df <- read_excel("Documents/Hult Master Fall 2020/Data Sience R/Air France Case Spreadsheet Supplement.xls", 
                     sheet = "DoubleClick")
View(AirFr_df)

#Structure of the Air France Data set
str(AirFr_df)

# Converting blanks to "NA"
AirFr_df[AirFr_df==""] <- NA

# Checking for missing values using the sapply() function 
sapply(AirFr_df, function(x) sum(is.na(x)))

#Creating a new column called Total Cost using the unique function
Total_Cost <- unique(AirFr_df$`Total Cost`)
# Creating a new column for Return on Advertising (ROA)
AirFr_df <- within(AirFr_df, ROA <- Amount/`Total Cost`)

#Creating a new column for Publisher Names
Publisher_Name <- unique(AirFr_df$`Publisher Name`)
...
for (i in 1:length(Publisher_Name)) {
  Sales <- c(Sales,sum(AirFr_df$Amount[which(AirFr_df[,2] == Publisher_Name[i])]))
  i <- i + 1
}
print(Sales)

#creating a matrix called Sales_Publisher
Sales_Publisher <- (cbind(Publisher_Name,as.numeric(Sales)))
